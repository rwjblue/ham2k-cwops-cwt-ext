  ...
  async function fetchBand(band: string, at: number): Promise<Spot[]> {
    ...
    for (let page = 0; page < maxPages; page++) {
      const response = await options.fetch(
        `${endpoint}?mode=CW&band=${band}&since=${since}&until=${until}&limit=${pageSize}&offset=${page * pageSize}`,
      ...
      reports.push(
        ...parseReports(payload.spots, options.source, at).filter((spot) => spot.band === band),
      )
      if (
