import { isCallsign, normalizeCall } from './callsign'
import { isCwtContest, membershipForExchange, normalizeKnownExchange } from './exchange'
import type { CallHistoryRecord, ParsedCallHistory, ParseIssue } from './types'
...
function contestComment(comment: string): string | undefined {
  ...
  if (
    /^(?:CWT|CWOPS|CWOPEN|CWO|MST|SST|NAQP(?:CW|SSB|RTTY)?|NS(?:LADDER|RTTY)?|ARRL[A-Z\d-]*|CQ[A-Z\d-]*|[A-Z]{2}QP|QSOPARTY(?: [A-Z]{2})?)$/.test(
      tag,
    )
  ) {
    return tag
...
/**
 * Parse a selected N1MM CWT call-history file. Fatal issues make the entire
...
export function parseCallHistory(text: string): ParsedCallHistory {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (line.startsWith('#')) {
      ...
      const updated = /^LastEdit\s*[,;]\s*(\d{4}-\d{2}-\d{2})$/i.exec(comment)
    ...
    const name = fields[order.indexOf('name')]?.trim() || undefined
    const rawNumber = fields[order.indexOf('exch1')]?.trim()
    ...
    const mergedName = name ?? prior?.name
    const mergedNumber = number ?? prior?.number
    records[call] = {
      call,
  ...
  if (associations.length && !associations.some(isCwtContest)) {
    addIssue(
      ...
      `File is associated with ${associations.join(', ')}, not CWT/CWOPS.`,
    )
  }
  if (!Object.keys(records).length)
    addIssue(0, 'error', 'empty-file', 'No valid callsign records found.')
  return {
    records,
    issues,
    associations,
